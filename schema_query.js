...
async function getTables() {
  try {
    ...
    const cols1 = await pool.request().query("SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='StoreInventory'");
    console.log("StoreInventory columns:", cols1.recordset);
    
    const cols2 = await pool.request().query("SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='Products'");
    console.log("Products columns:", cols2.recordset);

    const cols3 = await pool.request().query("SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='Categories'");
